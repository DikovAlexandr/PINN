"""
Module for active rectangular domain definitions.

This module inherits from :mod:`domainsBase` and defines:
1. An N-dimensional FBPINN domain with overlapping rectangular subdomains and
   its active subdomains,
2. The segments-in-model and models-in-segments index maps required when
   running and sharing NN outputs across neighbouring subdomains during training,
3. The normalisation values for each subdomain,
4. The window functions for each subdomain,
5. A sampler which samples torch points within the domain.

You can call :meth:`update_active` to update the active/fixed/inactive
subdomains during training. You can call :meth:`update_sampler` to choose
between a random or regular sampler.

This module is used by :mod:`main`.
"""
...
def itergrid(shape):
    ...
    For example, itergrid((2,3)) yields:
    (0, (0,0)); (1, (0,1)); (2, (0,2)); (3, (1,0)); (4, (1,1)); (5, (1,2))

    :param shape: Grid shape tuple.
    :type shape: tuple[int, ...]
    :return: Iterator over (index, coordinates) pairs.
    :rtype: iterator
    """
    return enumerate(itertools.product(*[np.arange(d) for d in shape]))


class ActiveRectangularDomainND(_RectangularDomainND):
    """
    N-dimensional domain with hyperrectangular subdomains.

    The active model grid can be controlled dynamically during training.
    ...
    :param scale: Scale parameter for window functions (default: 0.05).
    :type scale: float
    :param problem: Problem instance (optional).
    :type problem: object | None
    ...
    def __init__(self, subdomain_xs, subdomain_ws, scale=0.05, problem=None, device=None):
        super().__init__(subdomain_xs, subdomain_ws)

        self.N_MODELS = np.prod(self.nm)  # total number of models
        self.N_ORDERS = len(self.segments)  # 2**nd, the number of variations in constrained_axes
        self.onm = (self.N_ORDERS,) + self.nm  # like (1,)+(2,3)=(1,2,3,). shape: (self.N_ORDERS, nm)
        self.N_SEGMENTS = np.prod(self.onm)
        ...
        # This class must access problem class to filter in-(irregular)domain
        # points and sample on boundaries
        self.P = problem

    def _set_outside_filters(self):
        """
        Set filters for segments in model_segments which fall outside of the segment grid.

        Left over processing required from domainsBase._RectangularDomainND.
        """
        
        cs = []  # filters for each segment type
        for ioa, ms in enumerate(self.models_segments):  # (ne,nd,nm) ms
            # ioa: segment order number
            shape = np.array(
                self.segments[ioa].shape[2:]
            )  # (nd) shape of segment grid
            shape = shape.reshape(
                (1, self.nd) + (1,) * self.nd
            )  # (1,nd,1..) 1.. represents nd repetitions of 1
            c = (ms >= 0) & (ms < shape)  # (ne,nd,nm) (shape is broadcast)
            c = np.prod(c, axis=1).astype(
                bool
            )  # (ne,nm) collapse along ndim (i.e. has to be true across all nd dimensions)
            cs.append(c)
        self.cs = cs
    ...
    def _set_helpers(self):
        ...
        # wmins: overlap length on the left endpoint
        # wmaxs: overlap length on the right endpoint
        ...
        self.n = n  # defines D.n, shape: (prod(nm), 2, nd)
    ...
    def _get_neighbours(self, active):
        ...
        :param active: Active array.
        :type active: np.ndarray
        ...
        pad = np.pad(
            active.copy(), 1, mode="constant", constant_values=0
        )  # (nm+2) pads all dimensions with one zero either side
        ds = []
        for i in range(self.nd):
            sp, sn = [slice(1, -1, None)] * self.nd, [
                slice(1, -1, None)
            ] * self.nd  # cut out model grid
            sp[i] = slice(None, -2, None)
            sn[i] = slice(2, None, None)  # take previous/ next along current dimension
    ...
    def _get_window_functions(self, neighbours):
        ...
        :param neighbours: Neighbours array.
        :type neighbours: np.ndarray
        :return: List of window functions.
        :rtype: list[callable]
        """
        ...
        for im, ii in itergrid(self.nm):
            ...
            xmin, xmax = (
                self.xmins[sl],
                self.xmaxs[sl],
            )  # (nd) xmin, xmax of model at grid location
            wmin, wmax = (
                self.wmins[sl],
                self.wmaxs[sl],
            )  # (nd) wmin, wmax of model at grid location
            ...
            # Scale is by default 0.05
            w.append(
                construct_window_function_ND(
                    xmin, xmax, self.scale * wmin, self.scale * wmax
                )
            )
    ...
    def _get_isegs(self, ioa, ii):
        ...
        :param ioa: Segment order number.
        :type ioa: int
        :param ii: Grid index.
        :type ii: tuple
        :return: Tuple of (isegs, iis).
        ...
        if len(iis):  # nc can equal 0 if ioa > 0 and only one model spans a full subdomain axis
            isegs = np.ravel_multi_index(
                [np.array([ioa for _ in range(len(iis))])]
                + [iis[:, i] for i in range(self.nd)],
                self.onm,
            )
        else:
    ...
    def update_active(self, active=None):
        ...
        :param active: Active array (shape same as nm). Array element is 1->active, 2->fixed.
        :type active: np.ndarray | None
        """
        
        # Active's shape same as nm(grid of models), if array element is 1->active, 2->fixed
        if active is None:
            active = np.ones(self.nm, dtype=int)
        if active.shape != self.nm:
            raise Exception(
                f"ERROR: active shape {active.shape} does not equal model grid {self.nm}!"
            )
        ...
        c_active_neighbour = (
            np.sum(neighbours == 1, axis=(0, 1)) > 0
        )  # (nm) true if any neighbours are active
            ...
            if c_active[ii]:
                ...
                for ioa in range(self.N_ORDERS):
                    ...
                    for iseg, ii_ in zip(isegs, iis):
                        ...
                        i3 = i2 + np.prod(self.batch_ns[ioa][(slice(None),) + tuple(ii_)])
    ...
    def _set_torch_tensors(self, device):
        ...
        self.segments_torch = [
            totorch(segment) for segment in self.segments
        ]  # segments_torch is a copy of self.segments. Shape: (N_ORDERS, 2, nd, nm')
        self.n_torch = [
            (totorch(mu), totorch(sd)) for mu, sd in self.n
        ]  # n_torch is a copy of self.n. Shape: (prod(nm), 2, nd)

    def update_sampler(self, batch_size, random, bd_batch_size=0):
        """
        Set the batch size and type of the domain sampler.

        Be sure to use :meth:`update_active` to update index maps after this!

        :param batch_size: Batch size tuple.
        :type batch_size: tuple[int, ...]
        :param random: Whether to use random sampling.
        :type random: bool
        :param bd_batch_size: Boundary batch size (default: 0).
        :type bd_batch_size: int
        """
        
        if len(batch_size) != self.nd:
            raise Exception(f"ERROR: len(batch_size) != self.nd! ({batch_size})")
        ...
        for ioa in range(self.N_ORDERS):
            ...
            if random:
                n = np.round(
                    b_ * (segment[1] - segment[0]) / (lim_[1] - lim_[0])
                ).astype(int)  # (nd,nm) n number of points along each dim
            else:  # how many "grid points" in this interval
                i1 = np.ceil((segment[0] - lim_[0]) / d_).astype(
                    int
                )  # (nd,nm) i1 (d_ and lim_ are broadcast)
                i2 = np.floor((segment[1] - lim_[0]) / d_).astype(
                    int
                )  # (nd,nm) i2 (d_ and lim_ are broadcast)
                n = i2 - i1 + 1  # (nd,nm) n number of points along each dim
                i1s.append(i1)

            if not n.all():
                raise Exception("ERROR: batch_n has zeros in!")
            ns.append(n)
        ...
        # self.batch_ns has shape (N_ORDERS, nd, nm')

    def sample_segments(self):
        """
        Sample all the segments in the model, returning torch tensors.

        :return: Tuple of (xs, masks, boundaries, odata).
        :rtype: tuple
        """
        
        # Unit test the get_onm function
        '''
        print("in sample_segments:")
        print("onm:"+str(self.onm))
        print("N_SEGMENTS:%d"%self.N_SEGMENTS)
        for iseg in range(self.N_SEGMENTS):
            iii = np.unravel_index(iseg, self.onm)
            print("iseg, s[iseg], iii = "+str(iseg)+" "+str(bool(self.s[iseg]))+" "+str(iii))
            if self.s[iseg]:
                s = self.segments_torch[iii[0]][(slice(None),slice(None))+tuple(iii[1:])]# (2, nd)
                print(s)
                my_onm = self.get_onm((s[0]+s[1])/2)
                inferred_iseg = np.ravel_multi_index([[i] for i in my_onm],self.onm)
                print("inferred_iseg:%d"%inferred_iseg[0])
        '''
        # Sample Data Points
        if self.P is not None and hasattr(self.P, "sample_data"):
            odata = [list() for iseg in range(self.N_SEGMENTS)]
            od_xs = self.P.sample_data()
            for od_pt in od_xs:
                iii = self.get_onm(od_pt)
                iseg = np.ravel_multi_index([[i] for i in iii], self.onm)[0]
                if self.s[iseg]:  # if segment is in the active/fixed neighbour domain
                    odata[iseg].append(od_pt)
            for i in range(self.N_SEGMENTS):
                odata[i] = torch.tensor(
                    odata[i], dtype=torch.float32, device=self.device
                ).reshape(-1, self.nd)
        else:
            odata = None

        # Sample Boundary Points
        if self.P is not None and hasattr(self.P, "sample_bd"):
            boundaries = [list() for iseg in range(self.N_SEGMENTS)]
            bd_xs = self.P.sample_bd(self.bd_batch_size)
            for bd_pt in bd_xs:
                iii = self.get_onm(bd_pt)
                iseg = np.ravel_multi_index([[i] for i in iii], self.onm)[0]
                if self.s[iseg]:  # if segment is in the active/fixed neighbour domain
                    boundaries[iseg].append(bd_pt)
            for i in range(self.N_SEGMENTS):
                boundaries[i] = torch.tensor(
                    boundaries[i], dtype=torch.float32, device=self.device
                ).reshape(-1, self.nd)
        else:
            boundaries = None
        ...
        for iseg in range(self.N_SEGMENTS):  # for each segment
            if self.s[iseg]:  # if segment is in the active/fixed neighbour domain
                ...
                # Example: np.unravel_index(39,(8,2,3)) == (6,1,0)

                # Sample using TORCH
                if self.random:
                    n = self.batch_ns[iii[0]][(slice(None),) + tuple(iii[1:])]  # (nd)
                    s = self.segments_torch[iii[0]][
                        (slice(None), slice(None)) + tuple(iii[1:])
                    ]  # (2, nd)
                    x = s[0] + (s[1] - s[0]) * torch.rand(
                        np.prod(n), self.nd, device=self.device
                    )
                    # s[0] and s[1]-s[0] is broadcast along the np.prod(n) dimension.
                    ...
                    xs_ = [
                        li
                        + (i1i + torch.arange(ni, dtype=torch.float32, device=self.device))
                        * di
                        for li, i1i, ni, di in zip(self.batch_lim[0], i1, n, self.batch_d)
                    ]
                    x = torch.stack(torch.meshgrid(*xs_), -1).view(-1, self.nd)

                xs[iseg] = x  # xs[iseg] has shape (N_samples, nd)

        # Create Masks For Domain Points
        if self.P is not None and hasattr(self.P, "mask_x"):
            masks = [None for iseg in range(self.N_SEGMENTS)]
            for iseg in range(self.N_SEGMENTS):
                if self.s[iseg]:
                    masks[iseg] = self.P.mask_x(xs[iseg])
        else:
            masks = None
        ...
        return (
            ...
            boundaries,
            odata,
        )  # tensors in List "boundaries" might contain like torch.tensor([],size=(0,4))
