"""
Module for Chaotic equation problems (Kuramoto, GrayScott).
"""
...
class Kuramoto(_Problem):
    ...
    u(x,0) = cos(x) (1+sin(x))
    Ansatz: :math:`u(x,t) = u(x,0) + NN(x,t)\\tanh(t)`
    """
    ...
    def __init__(self):
        """
        Initialize the Kuramoto problem.
        """
    ...
    def get_gradients(self, x, y):
        ...
        jxxx = torch.autograd.grad(
            jxx, x, torch.ones_like(jx), create_graph=True
        )[0][:, 0:1]
        jxxxx = torch.autograd.grad(
            jxxx, x, torch.ones_like(jx), create_graph=True
        )[0][:, 0:1]
    ...
    def physics_loss(self, x, y, jt, jx, jxx, jxxxx):
        ...
        :param torch.Tensor x: Input coordinates.
        :param torch.Tensor y: Output values.
        :param torch.Tensor jt: Gradient w.r.t t.
        :param torch.Tensor jx: Gradient w.r.t x.
        :param torch.Tensor jxx: Second gradient w.r.t x.
        :param torch.Tensor jxxxx: Fourth gradient w.r.t x.
        :return: Physics loss value.
        :rtype: torch.Tensor
        """
        physics = self.t_scale * jt[:, 0] + self.alpha * y[:, 0] * jx[:, 0] + \
            self.beta * jxx[:, 0] + self.gamma * jxxxx[:, 0]
...
class GrayScott(_Problem):
    ...
    Ansatz: :math:`\\{u,v\\} = NN(x,y,t) \\tanh(t) + \\{u,v\\}(x,y,0)`
    """
    ...
    def __init__(self):
        """
        Initialize the GrayScott problem.
        """
    ...
    def physics_loss(self, x, y, ujt, vjt, nabla_u, nabla_v):
        ...
        :param torch.Tensor vjt: Gradient of v w.r.t t.
        :param torch.Tensor nabla_u: Laplacian of u.
        :param torch.Tensor nabla_v: Laplacian of v.
        :return: Physics loss value.
        :rtype: torch.Tensor
        """
        u, v = y[:, 0:1], y[:, 1:2]
    ...
    def boundary_condition(self, x, y, ujt, vjt, nabla_u, nabla_v, sd):
        ...
        nabla_u0 = -(
            (-2 * self.expdecay + self.expdecay**2 * 4 * (x[:, 0:1] + 0.05)) +
            (-2 * self.expdecay + self.expdecay**2 * 4 * (x[:, 1:2] + 0.02))
        ) * torch.exp(-self.expdecay * inner_u)
        
        nabla_v0 = (
            (-2 * self.expdecay + self.expdecay**2 * 4 * (x[:, 0:1] - 0.05)) +
            (-2 * self.expdecay + self.expdecay**2 * 4 * (x[:, 1:2] - 0.02))
        ) * torch.exp(-self.expdecay * inner_v)
