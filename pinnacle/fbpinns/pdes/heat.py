"""
Module for Heat equation problems.
"""
...
class HeatMultiscale(_Problem):
    ...
    Using the ansatz:
    :math:`u(x, y, t) = u(x, y, 0) + NN(x, y, t)\\tanh(wt)\\tanh(x)`
    :math:`\\times \\tanh(x-1)\\tanh(y)\\tanh(y-1)`
    """
    ...
    def __init__(self):
        """
        Initialize the HeatMultiscale problem.
        """
    ...
    def physics_loss(self, x, y, jt, jx, jy, jjx, jjy):
        ...
        :param torch.Tensor jjx: Second gradient with respect to x.
        :param torch.Tensor jjy: Second gradient with respect to y.
    ...
    def boundary_condition(self, x, y, jt, jx, jy, jjx, jjy, sd):
        ...
        sin_nx = torch.sin(self.nx * np.pi * x[:, 0:1])
        sin_y = torch.sin(np.pi * x[:, 1:2])
        cos_nx = torch.cos(self.nx * np.pi * x[:, 0:1])
        cos_y = torch.cos(np.pi * x[:, 1:2])
        ...
        u_new_x = (self.nx * np.pi) * cos_nx * sin_y + \
                  (y * jtx + jx * tx) * t0 * ty

        u_new_xx = (-(self.nx * np.pi) ** 2 * sin_nx * sin_y +
                    (y * jjtx + 2 * jx * jtx + jjx * tx) * t0 * ty)

        u_new_y = (np.pi) * cos_y * sin_nx + \
                  (y * jty + jy * ty) * t0 * tx

        u_new_yy = (-(np.pi) ** 2 * sin_y * sin_nx +
                    (y * jjty + 2 * jy * jty + jjy * ty) * t0 * tx)
        
        return u_new, u_new_t, u_new_x, u_new_y, u_new_xx, u_new_yy

    def exact_solution(self, x, batch_size):
        """
        Calculate the exact solution (or interpolate from reference).

        :param torch.Tensor x: Input coordinates.
        :param tuple batch_size: Batch size.
        :return: Tuple containing exact solution and ones (for compatibility).
        :rtype: tuple
        """
        # If no ref data, return dummy
        if not hasattr(self, 'ref_x'):
            return (torch.zeros((np.prod(batch_size), 1), device=x.device),) * 6
        ...
        x_mesh = [
            np.linspace(lim[0] + eps, lim[1] - eps, b)
...
class HeatComplex(_Problem):
    ...
    def __init__(self):
        """
        Initialize the HeatComplex problem.
        """
    ...
    def mask_x(self, x):
        """
        Create a mask for the input coordinates to handle holes.

        :param torch.Tensor x: Input coordinates of shape (b, nd).
        :return: Boolean mask of shape (nd).
        :rtype: torch.Tensor
        """
        x_cpu = x.detach().cpu()
        masks_all = torch.ones_like(x_cpu[:, 0], dtype=torch.bool)
        for centers, r in [
            (self.big_centers, self.big_r),
            (self.small_centers, self.small_r)
        ]:
            masks = [None] * len(centers)
            for i, center in enumerate(centers):
                masks[i] = torch.sum(
                    (x_cpu[:, :2] - torch.tensor(center))**2, dim=1
                ) > r**2
    ...
    def sample_bd(self, N_bd):
        ...
        for i in range(N_bd_circ):
            ...
            for center in self.big_centers:
                pts.append([
                    center[0] + self.big_r * np.cos(theta),
                    center[1] + self.big_r * np.sin(theta)
                ])
            for center in self.small_centers:
                pts.append([
                    center[0] + self.small_r * np.cos(theta),
                    center[1] + self.small_r * np.sin(theta)
                ])
    ...
    def bd_loss(self, x, y, jt, jx, jy, jjx, jjy):
        ...
        """
        def nv(x, c, r):
            # normal vector of holes point inwards
            return (
                torch.tensor([c[0], c[1]], dtype=torch.float32, device=x.device)
                - x[:, :2]
            ) / r
        ...
        d_edge = torch.sum(
            torch.stack([
                ...
                for cond, j in [
                    (isl, 0.1 - y + jx), (isr, 0.1 - y - jx),
                    (isb, 0.1 - y + jy), (ist, 0.1 - y - jy)
                ]
            ]),
            dim=0
        )
        ...
        is_smallcs = [
            np.isclose(
                (x[:, 0].detach().cpu() - center[0])**2 +
                (x[:, 1].detach().cpu() - center[1])**2,
                self.small_r**2
            )
            for center in self.small_centers
        ]
        ...
        d_small = torch.sum(
            torch.stack([
                torch.where(
                    cond,
                    1 - y[:, 0] - dot2d(nv(x, c, self.small_r), jx, jy),
                    0.
                )
                for cond, c in zip(
                    is_smallcs_, self.small_centers
                )
            ]),
            dim=0
        )
        ...
        d_big = torch.sum(
            torch.stack([
                torch.where(
                    cond,
                    5 - y[:, 0] - dot2d(nv(x, c, self.big_r), jx, jy),
                    0.
                )
                for cond, c in zip(
                    is_bigcs_, self.big_centers
                )
            ]),
            dim=0
        )
        
        # 1-y-dot2d leads to wrong tensorshape and wrong results
...
class HeatLongTime(_Problem):
    """
    Solves the heat equation for a long time period.
    """
    ...
    def __init__(self):
        """
        Initialize the HeatLongTime problem.
        """
    ...
    def physics_loss(self, x, y, jt, jjx, jjy):
        ...
        """
        physics = (
            jt - 0.001 * (jjx + jjy) - 5 * torch.sin(self.k * torch.square(y)) *
            (1 + 2 * torch.sin(x[:, 2:3] * np.pi / 4)) *
            torch.sin(self.m1 * x[:, 0:1]) * torch.sin(self.n1 * x[:, 1:2])
        )
        return losses.l2_loss(physics, 0)
...
class HeatDarcy(_Problem):
    ...
    def __init__(self):
        """
        Initialize the HeatDarcy problem.
        """
        self.bbox = [0, 1, 0, 1, 0, 5]
        self.d = (3, 1)
        ...
        coef_path = "../ref/heat_2d_coef_256.dat"
        grid_nums = 256
        
        if os.path.exists(coef_path):
            self.heat_2d_coef = np.loadtxt(coef_path)
            ...
            print(
                ...
                "Using deterministic synthetic coefficient field (no randomness)."
            )
            xs = np.linspace(0, 1, grid_nums)
            ys = np.linspace(0, 1, grid_nums)
            self.coef_in = (xs, ys)
            xx, yy = np.meshgrid(xs, ys, indexing="ij")
            # Strictly positive coefficient field; smooth but nontrivial.
            self.coef_values = (
                1.0
                + 0.35 * np.sin(2 * np.pi * xx) * np.cos(2 * np.pi * yy)
                + 0.15 * np.cos(4 * np.pi * xx + 0.3) * np.sin(2 * np.pi * yy - 0.2)
            ).astype(np.float64)
    ...
    def physics_loss(self, x, y, jt, jx, jy, jjx, jjy):
        ...
        intp_result = scipy.interpolate.interpn(
            ref_pts,
            self.coef_values.astype(np.float64),
            x[:, 0:2].detach().cpu().numpy().astype(np.float64)
        )
        coef_x = torch.tensor(intp_result.astype(np.float32), device=x.device)

        def f(x):
            return self.A * torch.sin(self.m1 * np.pi * x[:, 0]) * \
                torch.sin(self.m2 * np.pi * x[:, 1]) * \
                torch.sin(self.m3 * np.pi * x[:, 2])
...
class HeatND(_Problem):
    """
    Solves the N-dimensional Heat equation.
    """
    ...
    def __init__(self, spacedim=5):
        """
        Initialize the HeatND problem.

        :param int spacedim: spatial dimension.
        """
    ...
    def physics_loss(self, x, y, j, jjsum):
        ...
        :param torch.Tensor x: Input coordinates.
        :param torch.Tensor y: Output values.
        :param torch.Tensor j: Gradient with respect to all dims.
        ...
        f = -(1 / (self.d[0] - 1)) * rsquare * torch.exp(
            0.5 * rsquare + x[:, -1:]
        )
        return losses.l2_loss(
            jt, (1 / (self.d[0] - 1)) * jjsum + f
        )
    ...
    def exact_solution(self, x, batch_size):
        ...
        """
        return (
            self.exact_solution_(x),
            torch.ones((np.prod(batch_size), self.d[0]), device=x.device),
...
class HeatMultiscaleExact(_Problem):
    ...
    def __init__(self):
        """
        Initialize the HeatMultiscaleExact problem.
        """
        self.bbox = [0, 1, 0, 1, 0, 5]
        self.d = (3, 1)  # input dim order: x, y, t
        self.nx = 20
    ...
    def boundary_condition(self, x, y, jt, jx, jy, jjx, jjy, sd):
        ...
        sin_nx = torch.sin(self.nx * np.pi * x[:, 0:1])
        sin_ny = torch.sin(self.ny * np.pi * x[:, 1:2])
        cos_nx = torch.cos(self.nx * np.pi * x[:, 0:1])
        cos_ny = torch.cos(self.ny * np.pi * x[:, 1:2])
        
        u_new = sin_nx * sin_ny + y * t0 * tx * ty
        
        u_new_t = (jt * t0 + y * jt0) * tx * ty
        
        u_new_x = (
            (self.nx * np.pi) * cos_nx * sin_ny
            + (y * jtx + jx * tx) * t0 * ty
        )

        u_new_xx = (
            -(self.nx * np.pi) ** 2 * sin_nx * sin_ny
            + (y * jjtx + 2 * jx * jtx + jjx * tx) * t0 * ty
        )

        u_new_y = (
            (self.ny * np.pi) * cos_ny * sin_nx
            + (y * jty + jy * ty) * t0 * tx
        )

        u_new_yy = (
            -(self.ny * np.pi) ** 2 * sin_ny * sin_nx
            + (y * jjty + 2 * jy * jty + jjy * ty) * t0 * tx
        )
    ...
    def exact_solution(self, x, batch_size):
        ...
        y_exact = torch.sin(self.nx * np.pi * x[:, 0:1]) * \
            torch.sin(self.ny * np.pi * x[:, 1:2]) * \
            torch.exp(-decay * x[:, 2:3])
        y_exact.to(x.device)

        ones = torch.ones(
            (np.prod(batch_size), 1), device=x.device
        )
        return (y_exact,) + (ones,) * 5
