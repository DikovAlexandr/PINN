...
class VarianceScalingStacked:
    ...
    def __init__(
        ...
        self.seed = seed
        self.dtype = config.real(torch)

    def __call__(self, shape, dtype=None):
        """Generate initialized tensor using PyTorch backend."""
        if dtype is None:
            dtype = self.dtype
        scale = self.scale
        ...
        # Use PyTorch for initialization
        if self.distribution == "normal" or self.distribution == "truncated_normal":
            # constant taken from scipy.stats.truncnorm.std(a=-2, b=2, loc=0., scale=1.)
            stddev = math.sqrt(scale) / 0.87962566103423978
            tensor = torch.empty(shape, dtype=dtype)
            torch.nn.init.trunc_normal_(tensor, mean=0.0, std=stddev, a=-2*stddev, b=2*stddev)
            return tensor
        elif self.distribution == "untruncated_normal":
            stddev = math.sqrt(scale)
            return torch.normal(0.0, stddev, size=shape).to(dtype)
        else:
...
def initializer_dict_torch():
    """PyTorch initializer dictionary."""
    return {
        ...
        # Initializers of stacked DeepONet (PyTorch implementation)
